...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= str.length; i++) {
    let wordSlice = str.slice(i);
    result.push(wordSlice);
  }
  return result;
};
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  recipe.ingredients.forEach(compo => {
    let indexOfFirstSpace = compo.indexOf(' ');
    let sliceTheNo = compo.slice(indexOfFirstSpace + 1);
    let indexOfSecondSpace = sliceTheNo.indexOf(' ');
    let componant = sliceTheNo.slice(indexOfSecondSpace + 1);
    result.push(componant);
  })
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(compo => {
    var res = compo.split(" ");
    var a = res.slice(2, compo.length);
    var aJoin = a.join(" ");
    result.push(aJoin);
  })
  return result;
...
const stepActions = (recipe) => {
  let result = [];
  // Solution code here...
  recipe.steps.forEach(compo => {
    result.push(compo.split(' ')[0]);
  });
  return result;
};
...
const removeEvenValues = (arr) => {
  // Solution code here...
  let arrayNo = [];
  arr.forEach(no => arrayNo.push(no));
  arrayNo.forEach((no) => {
    if (no % 2 === 0) {
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters < 0 ){
    return str ;
  }else if(numberOfCharacters > numberOfCharacters.length){
    return '';
  }else{
