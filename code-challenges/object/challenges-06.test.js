...
const getCourseKeys = (obj) => {
    // Solution code here...
    let keyy = Object.keys(courseInfo);
    return keyy;
};
...
const hasChildrenEntries = (arr, character) => {
    ...
    let bool = false;
    arr.forEach(house => {
        Object.entries(house).forEach(val => {
            if (val[1] === character) {
...
const houseSize = (arr) => {
    ...
    // Solution code here...
    arr.forEach(element => {
        const house = element.house;
        let members = 1 + element.children.length;
        members += element.spouse ? 1 : 0;
...
const houseSurvivors = (arr) => {
    ...
    let countt = 0;
    arr.forEach(val => {
        let count = val.children.length;
        countt = 1 + count + countt;
        survivors.push({
            house: val.house,
            members: countt
        })
