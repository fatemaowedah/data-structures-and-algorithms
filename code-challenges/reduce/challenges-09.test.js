...
const calculateAverage = (arr) => {
  // Solution code here...
  let nu = arr.reduce((accumulator,value) => {
    accumulator['count']++;
    accumulator['sum']=accumulator['sum']+value;
    return accumulator;
  },{count: 0, sum: 0});
  return nu.sum/nu.count;
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  const stat = arr.reduce((accumulator, value) => {
...
const extractChildren = (arr) => {
  // Solution code here...
  return arr
    .filter(element => /a/.test(element.name)).reduce((accumulator, value) => {
      if (value.children) {
