...
const count = (target, input) => {
  ...
  input.forEach(element => {
    sumation = sumation + element.reduce((accumelator, value) => {
      if (value === target) accumelator++;
      return accumelator;
    }, 0);
  });
  return sumation;
...
const totalSum = (input) => {
  // Solution code here...
  let newArray = input.reduce((acc,val) =>{
